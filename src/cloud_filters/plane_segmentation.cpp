...
#define PCL_INSTANTIATE_DRLPlaneSegmentation(T) template class PCL_EXPORTS dynamic_robot_localization::PlaneSegmentation<T>;
PCL_INSTANTIATE(DRLPlaneSegmentation, DRL_POINT_TYPES)
#endif
// >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>   </template instantiations>  <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
