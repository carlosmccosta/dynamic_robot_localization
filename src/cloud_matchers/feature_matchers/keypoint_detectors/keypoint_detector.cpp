...
#define PCL_INSTANTIATE_DRLKeypointDetector(T) template class PCL_EXPORTS dynamic_robot_localization::KeypointDetector<T>;
PCL_INSTANTIATE(DRLKeypointDetector, DRL_POINT_TYPES)
#endif
// >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>   </template instantiations>  <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
