...
bool RegistrationCovariancePointToPoint3D<PointT>::computeRegistrationCovariance(const pcl::PointCloud<PointT>& reference_cloud_correspondences_map_frame, const pcl::PointCloud<PointT>& ambient_cloud_orrespondences_map_frame,
		const Eigen::Matrix4f& registration_corrections, const Eigen::Transform<float, 3, Eigen::Affine>& transform_from_map_cloud_data_to_sensor_origin, const std::string& sensor_frame_id, Eigen::MatrixXd& covariance_out, double sensor_mean_noise) {
	if (reference_cloud_correspondences_map_frame.empty() || ambient_cloud_orrespondences_map_frame.empty()) {
		return false;
	}

	pcl::PointCloud<PointT> reference_cloud_correspondences;
	pcl::PointCloud<PointT> ambient_cloud_orrespondences;

	pcl::transformPointCloud(reference_cloud_correspondences_map_frame, reference_cloud_correspondences, transform_from_map_cloud_data_to_sensor_origin);
	pcl::transformPointCloud(ambient_cloud_orrespondences_map_frame, ambient_cloud_orrespondences, transform_from_map_cloud_data_to_sensor_origin);

	reference_cloud_correspondences.header = ambient_cloud_orrespondences_map_frame.header;
	ambient_cloud_orrespondences.header = ambient_cloud_orrespondences_map_frame.header;
	reference_cloud_correspondences.header.frame_id = sensor_frame_id;
	ambient_cloud_orrespondences.header.frame_id = sensor_frame_id;

	double x = registration_corrections(0, 3);
	double y = registration_corrections(1, 3);
	double z = registration_corrections(2, 3);
	double yaw = atan2f(registration_corrections(2, 1), registration_corrections(2, 2));
	double pitch = asinf(-registration_corrections(2, 0));
	double roll = atan2f(registration_corrections(1, 0), registration_corrections(0, 0));
	...
	for (size_t s = 0; s < ambient_cloud_orrespondences_size; ++s) {
		double pix = ambient_cloud_orrespondences.points[s].x;
		double piy = ambient_cloud_orrespondences.points[s].y;
		double piz = ambient_cloud_orrespondences.points[s].z;
		double qix = reference_cloud_correspondences.points[s].x;
		double qiy = reference_cloud_correspondences.points[s].y;
		double qiz = reference_cloud_correspondences.points[s].z;
		...
		d2J_dbdc = (2 * piy * cos_yaw * cos_pitch * cos_roll - 2 * piz * cos_yaw * cos_pitch * sin_roll) * (x - qix - piy * (cos_roll * sin_yaw - cos_yaw * sin_pitch * sin_roll) + piz * (sin_yaw * sin_roll + cos_yaw * cos_roll * sin_pitch) + pix * cos_yaw * cos_pitch) + (2 * piy * cos_pitch * cos_roll * sin_yaw - 2 * piz * cos_pitch * sin_yaw * sin_roll) * (y - qiy + piy * (cos_yaw * cos_roll + sin_yaw * sin_pitch * sin_roll) - piz * (cos_yaw * sin_roll - cos_roll * sin_yaw * sin_pitch) + pix * cos_pitch * sin_yaw) - (2 * piy * cos_pitch * cos_roll - 2 * piz * cos_pitch * sin_roll) * (pix * cos_pitch + piz * cos_roll * sin_pitch + piy * sin_pitch * sin_roll) - (2 * piy * cos_roll * sin_pitch - 2 * piz * sin_pitch * sin_roll) * (z - qiz - pix * sin_pitch + piz * cos_pitch * cos_roll + piy * cos_pitch * sin_roll) + (2 * piy * (sin_yaw * sin_roll + cos_yaw * cos_roll * sin_pitch) + 2 * piz * (cos_roll * sin_yaw - cos_yaw * sin_pitch * sin_roll)) * (piz * cos_yaw * cos_pitch * cos_roll - pix * cos_yaw * sin_pitch + piy * cos_yaw * cos_pitch * sin_roll) - (2 * piy * (cos_yaw * sin_roll - cos_roll * sin_yaw * sin_pitch) + 2 * piz * (cos_yaw * cos_roll + sin_yaw * sin_pitch * sin_roll)) * (piz * cos_pitch * cos_roll * sin_yaw - pix * sin_yaw * sin_pitch + piy * cos_pitch * sin_yaw * sin_roll);
		d2J_dcdb = (2 * piy * cos_yaw * cos_pitch * cos_roll - 2 * piz * cos_yaw * cos_pitch * sin_roll) * (x - qix - piy * (cos_roll * sin_yaw - cos_yaw * sin_pitch * sin_roll) + piz * (sin_yaw * sin_roll + cos_yaw * cos_roll * sin_pitch) + pix * cos_yaw * cos_pitch) + (2 * piy * cos_pitch * cos_roll * sin_yaw - 2 * piz * cos_pitch * sin_yaw * sin_roll) * (y - qiy + piy * (cos_yaw * cos_roll + sin_yaw * sin_pitch * sin_roll) - piz * (cos_yaw * sin_roll - cos_roll * sin_yaw * sin_pitch) + pix * cos_pitch * sin_yaw) - (piy * cos_pitch * cos_roll - piz * cos_pitch * sin_roll) * (2 * pix * cos_pitch + 2 * piz * cos_roll * sin_pitch + 2 * piy * sin_pitch * sin_roll) - (2 * piy * cos_roll * sin_pitch - 2 * piz * sin_pitch * sin_roll) * (z - qiz - pix * sin_pitch + piz * cos_pitch * cos_roll + piy * cos_pitch * sin_roll) + (piy * (sin_yaw * sin_roll + cos_yaw * cos_roll * sin_pitch) + piz * (cos_roll * sin_yaw - cos_yaw * sin_pitch * sin_roll)) * (2 * piz * cos_yaw * cos_pitch * cos_roll - 2 * pix * cos_yaw * sin_pitch + 2 * piy * cos_yaw * cos_pitch * sin_roll) - (piy * (cos_yaw * sin_roll - cos_roll * sin_yaw * sin_pitch) + piz * (cos_yaw * cos_roll + sin_yaw * sin_pitch * sin_roll)) * (2 * piz * cos_pitch * cos_roll * sin_yaw - 2 * pix * sin_yaw * sin_pitch + 2 * piy * cos_pitch * sin_yaw * sin_roll);
		d2J_dcda = (2 * piy * (cos_yaw * sin_roll - cos_roll * sin_yaw * sin_pitch) + 2 * piz * (cos_yaw * cos_roll + sin_yaw * sin_pitch * sin_roll)) * (x - qix - piy * (cos_roll * sin_yaw - cos_yaw * sin_pitch * sin_roll) + piz * (sin_yaw * sin_roll + cos_yaw * cos_roll * sin_pitch) + pix * cos_yaw * cos_pitch) - (piy * (sin_yaw * sin_roll + cos_yaw * cos_roll * sin_pitch) + piz * (cos_roll * sin_yaw - cos_yaw * sin_pitch * sin_roll)) * (2 * piy * (cos_yaw * cos_roll + sin_yaw * sin_pitch * sin_roll) - 2 * piz * (cos_yaw * sin_roll - cos_roll * sin_yaw * sin_pitch) + 2 * pix * cos_pitch * sin_yaw) - (piy * (cos_yaw * sin_roll - cos_roll * sin_yaw * sin_pitch) + piz * (cos_yaw * cos_roll + sin_yaw * sin_pitch * sin_roll)) * (2 * piz * (sin_yaw * sin_roll + cos_yaw * cos_roll * sin_pitch) - 2 * piy * (cos_roll * sin_yaw - cos_yaw * sin_pitch * sin_roll) + 2 * pix * cos_yaw * cos_pitch) + (2 * piy * (sin_yaw * sin_roll + cos_yaw * cos_roll * sin_pitch) + 2 * piz * (cos_roll * sin_yaw - cos_yaw * sin_pitch * sin_roll)) * (y - qiy + piy * (cos_yaw * cos_roll + sin_yaw * sin_pitch * sin_roll) - piz * (cos_yaw * sin_roll - cos_roll * sin_yaw * sin_pitch) + pix * cos_pitch * sin_yaw);
		d2J_dadc = (2 * piy * (cos_yaw * sin_roll - cos_roll * sin_yaw * sin_pitch) + 2 * piz * (cos_yaw * cos_roll + sin_yaw * sin_pitch * sin_roll)) * (x - qix - piy * (cos_roll * sin_yaw - cos_yaw * sin_pitch * sin_roll) + piz * (sin_yaw * sin_roll + cos_yaw * cos_roll * sin_pitch) + pix * cos_yaw * cos_pitch) - (2 * piy * (sin_yaw * sin_roll + cos_yaw * cos_roll * sin_pitch) + 2 * piz * (cos_roll * sin_yaw - cos_yaw * sin_pitch * sin_roll)) * (piy * (cos_yaw * cos_roll + sin_yaw * sin_pitch * sin_roll) - piz * (cos_yaw * sin_roll - cos_roll * sin_yaw * sin_pitch) + pix * cos_pitch * sin_yaw) - (2 * piy * (cos_yaw * sin_roll - cos_roll * sin_yaw * sin_pitch) + 2 * piz * (cos_yaw * cos_roll + sin_yaw * sin_pitch * sin_roll)) * (piz * (sin_yaw * sin_roll + cos_yaw * cos_roll * sin_pitch) - piy * (cos_roll * sin_yaw - cos_yaw * sin_pitch * sin_roll) + pix * cos_yaw * cos_pitch) + (2 * piy * (sin_yaw * sin_roll + cos_yaw * cos_roll * sin_pitch) + 2 * piz * (cos_roll * sin_yaw - cos_yaw * sin_pitch * sin_roll)) * (y - qiy + piy * (cos_yaw * cos_roll + sin_yaw * sin_pitch * sin_roll) - piz * (cos_yaw * sin_roll - cos_roll * sin_yaw * sin_pitch) + pix * cos_pitch * sin_yaw);
	...
	for (size_t k = 0; k < ambient_cloud_orrespondences_size; ++k) {
		...
		d2J_dpix_da = cos_pitch * sin_yaw * (2 * piz * (sin_yaw * sin_roll + cos_yaw * cos_roll * sin_pitch) - 2 * piy * (cos_roll * sin_yaw - cos_yaw * sin_pitch * sin_roll) + 2 * pix * cos_yaw * cos_pitch) - cos_yaw * cos_pitch * (2 * piy * (cos_yaw * cos_roll + sin_yaw * sin_pitch * sin_roll) - 2 * piz * (cos_yaw * sin_roll - cos_roll * sin_yaw * sin_pitch) + 2 * pix * cos_pitch * sin_yaw) - 2 * cos_pitch * sin_yaw * (x - qix - piy * (cos_roll * sin_yaw - cos_yaw * sin_pitch * sin_roll) + piz * (sin_yaw * sin_roll + cos_yaw * cos_roll * sin_pitch) + pix * cos_yaw * cos_pitch) + 2 * cos_yaw * cos_pitch * (y - qiy + piy * (cos_yaw * cos_roll + sin_yaw * sin_pitch * sin_roll) - piz * (cos_yaw * sin_roll - cos_roll * sin_yaw * sin_pitch) + pix * cos_pitch * sin_yaw);
		d2J_dpix_db = sin_pitch * (2 * pix * cos_pitch + 2 * piz * cos_roll * sin_pitch + 2 * piy * sin_pitch * sin_roll) - 2 * cos_pitch * (z - qiz - pix * sin_pitch + piz * cos_pitch * cos_roll + piy * cos_pitch * sin_roll) + cos_yaw * cos_pitch * (2 * piz * cos_yaw * cos_pitch * cos_roll - 2 * pix * cos_yaw * sin_pitch + 2 * piy * cos_yaw * cos_pitch * sin_roll) - 2 * sin_yaw * sin_pitch * (y - qiy + piy * (cos_yaw * cos_roll + sin_yaw * sin_pitch * sin_roll) - piz * (cos_yaw * sin_roll - cos_roll * sin_yaw * sin_pitch) + pix * cos_pitch * sin_yaw) + cos_pitch * sin_yaw * (2 * piz * cos_pitch * cos_roll * sin_yaw - 2 * pix * sin_yaw * sin_pitch + 2 * piy * cos_pitch * sin_yaw * sin_roll) - 2 * cos_yaw * sin_pitch * (x - qix - piy * (cos_roll * sin_yaw - cos_yaw * sin_pitch * sin_roll) + piz * (sin_yaw * sin_roll + cos_yaw * cos_roll * sin_pitch) + pix * cos_yaw * cos_pitch);
		...
		d2J_dpiy_da = (cos_yaw * cos_roll + sin_yaw * sin_pitch * sin_roll) * (2 * piz * (sin_yaw * sin_roll + cos_yaw * cos_roll * sin_pitch) - 2 * piy * (cos_roll * sin_yaw - cos_yaw * sin_pitch * sin_roll) + 2 * pix * cos_yaw * cos_pitch) + (cos_roll * sin_yaw - cos_yaw * sin_pitch * sin_roll) * (2 * piy * (cos_yaw * cos_roll + sin_yaw * sin_pitch * sin_roll) - 2 * piz * (cos_yaw * sin_roll - cos_roll * sin_yaw * sin_pitch) + 2 * pix * cos_pitch * sin_yaw) - (2 * cos_yaw * cos_roll + 2 * sin_yaw * sin_pitch * sin_roll) * (x - qix - piy * (cos_roll * sin_yaw - cos_yaw * sin_pitch * sin_roll) + piz * (sin_yaw * sin_roll + cos_yaw * cos_roll * sin_pitch) + pix * cos_yaw * cos_pitch) - (2 * cos_roll * sin_yaw - 2 * cos_yaw * sin_pitch * sin_roll) * (y - qiy + piy * (cos_yaw * cos_roll + sin_yaw * sin_pitch * sin_roll) - piz * (cos_yaw * sin_roll - cos_roll * sin_yaw * sin_pitch) + pix * cos_pitch * sin_yaw);
		d2J_dpiy_db = (cos_yaw * cos_roll + sin_yaw * sin_pitch * sin_roll) * (2 * piz * cos_pitch * cos_roll * sin_yaw - 2 * pix * sin_yaw * sin_pitch + 2 * piy * cos_pitch * sin_yaw * sin_roll) - (cos_roll * sin_yaw - cos_yaw * sin_pitch * sin_roll) * (2 * piz * cos_yaw * cos_pitch * cos_roll - 2 * pix * cos_yaw * sin_pitch + 2 * piy * cos_yaw * cos_pitch * sin_roll) - 2 * sin_pitch * sin_roll * (z - qiz - pix * sin_pitch + piz * cos_pitch * cos_roll + piy * cos_pitch * sin_roll) - cos_pitch * sin_roll * (2 * pix * cos_pitch + 2 * piz * cos_roll * sin_pitch + 2 * piy * sin_pitch * sin_roll) + 2 * cos_yaw * cos_pitch * sin_roll * (x - qix - piy * (cos_roll * sin_yaw - cos_yaw * sin_pitch * sin_roll) + piz * (sin_yaw * sin_roll + cos_yaw * cos_roll * sin_pitch) + pix * cos_yaw * cos_pitch) + 2 * cos_pitch * sin_yaw * sin_roll * (y - qiy + piy * (cos_yaw * cos_roll + sin_yaw * sin_pitch * sin_roll) - piz * (cos_yaw * sin_roll - cos_roll * sin_yaw * sin_pitch) + pix * cos_pitch * sin_yaw);
		d2J_dpiy_dc = (2 * sin_yaw * sin_roll + 2 * cos_yaw * cos_roll * sin_pitch) * (x - qix - piy * (cos_roll * sin_yaw - cos_yaw * sin_pitch * sin_roll) + piz * (sin_yaw * sin_roll + cos_yaw * cos_roll * sin_pitch) + pix * cos_yaw * cos_pitch) - (2 * cos_yaw * sin_roll - 2 * cos_roll * sin_yaw * sin_pitch) * (y - qiy + piy * (cos_yaw * cos_roll + sin_yaw * sin_pitch * sin_roll) - piz * (cos_yaw * sin_roll - cos_roll * sin_yaw * sin_pitch) + pix * cos_pitch * sin_yaw) - (cos_yaw * cos_roll + sin_yaw * sin_pitch * sin_roll) * (2 * piy * (cos_yaw * sin_roll - cos_roll * sin_yaw * sin_pitch) + 2 * piz * (cos_yaw * cos_roll + sin_yaw * sin_pitch * sin_roll)) - (cos_roll * sin_yaw - cos_yaw * sin_pitch * sin_roll) * (2 * piy * (sin_yaw * sin_roll + cos_yaw * cos_roll * sin_pitch) + 2 * piz * (cos_roll * sin_yaw - cos_yaw * sin_pitch * sin_roll)) + 2 * cos_pitch * cos_roll * (z - qiz - pix * sin_pitch + piz * cos_pitch * cos_roll + piy * cos_pitch * sin_roll) + cos_pitch * sin_roll * (2 * piy * cos_pitch * cos_roll - 2 * piz * cos_pitch * sin_roll);
		...
		d2J_dpiz_da = (2 * cos_yaw * sin_roll - 2 * cos_roll * sin_yaw * sin_pitch) * (x - qix - piy * (cos_roll * sin_yaw - cos_yaw * sin_pitch * sin_roll) + piz * (sin_yaw * sin_roll + cos_yaw * cos_roll * sin_pitch) + pix * cos_yaw * cos_pitch) - (sin_yaw * sin_roll + cos_yaw * cos_roll * sin_pitch) * (2 * piy * (cos_yaw * cos_roll + sin_yaw * sin_pitch * sin_roll) - 2 * piz * (cos_yaw * sin_roll - cos_roll * sin_yaw * sin_pitch) + 2 * pix * cos_pitch * sin_yaw) - (cos_yaw * sin_roll - cos_roll * sin_yaw * sin_pitch) * (2 * piz * (sin_yaw * sin_roll + cos_yaw * cos_roll * sin_pitch) - 2 * piy * (cos_roll * sin_yaw - cos_yaw * sin_pitch * sin_roll) + 2 * pix * cos_yaw * cos_pitch) + (2 * sin_yaw * sin_roll + 2 * cos_yaw * cos_roll * sin_pitch) * (y - qiy + piy * (cos_yaw * cos_roll + sin_yaw * sin_pitch * sin_roll) - piz * (cos_yaw * sin_roll - cos_roll * sin_yaw * sin_pitch) + pix * cos_pitch * sin_yaw);
		d2J_dpiz_db = (sin_yaw * sin_roll + cos_yaw * cos_roll * sin_pitch) * (2 * piz * cos_yaw * cos_pitch * cos_roll - 2 * pix * cos_yaw * sin_pitch + 2 * piy * cos_yaw * cos_pitch * sin_roll) - (cos_yaw * sin_roll - cos_roll * sin_yaw * sin_pitch) * (2 * piz * cos_pitch * cos_roll * sin_yaw - 2 * pix * sin_yaw * sin_pitch + 2 * piy * cos_pitch * sin_yaw * sin_roll) - 2 * cos_roll * sin_pitch * (z - qiz - pix * sin_pitch + piz * cos_pitch * cos_roll + piy * cos_pitch * sin_roll) - cos_pitch * cos_roll * (2 * pix * cos_pitch + 2 * piz * cos_roll * sin_pitch + 2 * piy * sin_pitch * sin_roll) + 2 * cos_yaw * cos_pitch * cos_roll * (x - qix - piy * (cos_roll * sin_yaw - cos_yaw * sin_pitch * sin_roll) + piz * (sin_yaw * sin_roll + cos_yaw * cos_roll * sin_pitch) + pix * cos_yaw * cos_pitch) + 2 * cos_pitch * cos_roll * sin_yaw * (y - qiy + piy * (cos_yaw * cos_roll + sin_yaw * sin_pitch * sin_roll) - piz * (cos_yaw * sin_roll - cos_roll * sin_yaw * sin_pitch) + pix * cos_pitch * sin_yaw);
		d2J_dpiz_dc = (2 * cos_roll * sin_yaw - 2 * cos_yaw * sin_pitch * sin_roll) * (x - qix - piy * (cos_roll * sin_yaw - cos_yaw * sin_pitch * sin_roll) + piz * (sin_yaw * sin_roll + cos_yaw * cos_roll * sin_pitch) + pix * cos_yaw * cos_pitch) - (2 * cos_yaw * cos_roll + 2 * sin_yaw * sin_pitch * sin_roll) * (y - qiy + piy * (cos_yaw * cos_roll + sin_yaw * sin_pitch * sin_roll) - piz * (cos_yaw * sin_roll - cos_roll * sin_yaw * sin_pitch) + pix * cos_pitch * sin_yaw) + (sin_yaw * sin_roll + cos_yaw * cos_roll * sin_pitch) * (2 * piy * (sin_yaw * sin_roll + cos_yaw * cos_roll * sin_pitch) + 2 * piz * (cos_roll * sin_yaw - cos_yaw * sin_pitch * sin_roll)) + (cos_yaw * sin_roll - cos_roll * sin_yaw * sin_pitch) * (2 * piy * (cos_yaw * sin_roll - cos_roll * sin_yaw * sin_pitch) + 2 * piz * (cos_yaw * cos_roll + sin_yaw * sin_pitch * sin_roll)) + cos_pitch * cos_roll * (2 * piy * cos_pitch * cos_roll - 2 * piz * cos_pitch * sin_roll) - 2 * cos_pitch * sin_roll * (z - qiz - pix * sin_pitch + piz * cos_pitch * cos_roll + piy * cos_pitch * sin_roll);
		d2J_dqix_dx = -2;
		d2J_dqix_dy = 0;
		d2J_dqix_dz = 0;
	...
	cov_z = sensor_mean_noise * Eigen::MatrixXd::Identity(6 * ambient_cloud_orrespondences_size, 6 * ambient_cloud_orrespondences_size);
	covariance_out = d2J_dX2.inverse() * d2J_dZdX * cov_z * d2J_dZdX.transpose() * d2J_dX2.inverse();

	if (RegistrationCovarianceEstimator<PointT>::cloud_publisher_reference_cloud_) {
		RegistrationCovarianceEstimator<PointT>::cloud_publisher_reference_cloud_->publishPointCloud(reference_cloud_correspondences);
	}

	if (RegistrationCovarianceEstimator<PointT>::cloud_publisher_ambient_cloud_) {
		RegistrationCovarianceEstimator<PointT>::cloud_publisher_ambient_cloud_->publishPointCloud(ambient_cloud_orrespondences);
	}
