...
void VoxelGrid<PointT>::setupConfigurationFromParameterServer(ros::NodeHandlePtr& node_handle, ros::NodeHandlePtr& private_node_handle, std::string configuration_namespace) {
	...
	cloud_publisher->setParameterServerArgumentToLoadTopicName(configuration_namespace + "voxel_grid_filtered_cloud_publish_topic");
...
void VoxelGrid<PointT>::filter(const typename pcl::PointCloud<PointT>::Ptr& input_cloud, typename pcl::PointCloud<PointT>::Ptr& output_cloud) {
	...
	ROS_DEBUG_STREAM("VoxelGrid reduced point cloud from " << number_of_points_in_input_cloud << " points to " << output_cloud->size() << " points");
}
